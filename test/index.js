var vlc = require('../index')();
    nock = require('nock'),
    chai = require('chai'),
    assert = chai.assert;

var base = 'http://vlc-api.test';
vlc._base = base;
...
describe('vlc-api', function () {
  ...
  it('should browse for this particular uri without error', function (done) {
    nock(base)
      .get('/requests/browse.json?uri=http%3A%2F%2Fyoutu.be%2FYSgAc6ytFOs')
      .reply(200);
    vlc.browse('http://youtu.be/YSgAc6ytFOs', done);
  })
})

describe('request client api', function () {
  it('should throws an error if Vlc need a password', function () {
    nock(base)
      .get('/requests/3.json')
      .reply(401);
    vlc.request(3, function (err, res) {
      assert.equal(err, 'Error: HTTP status 401');
    });
  })
  it('should throws an error if connect is not found', function () {
    var opts = {}
