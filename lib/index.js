...
var Client = module.exports = function (opts) {
  ...
  this.status.sort = function (mode, order, cb) {
    ...
    if (typeof mode == 'string') {
      modes = ['id', 'name', null, 'author', null, 'random', null, 'track'];
      mode =  modes.indexOf(mode.toLowerCase())
      ;
      if (mode == -1) {
        // No specified mode! Error time.
        err = err || new Error('Modes are: `' + _.compact(modes).join('`, `') + '`.');
      }
    }
...
Client.prototype.request = function (resource, opts, cb) {
  var client = this;
  if (_.isFunction(resource) || _.isUndefined(resource)) {
    throw new Error('First argument to Client#request should be a resource.');
  }
  if (_.isFunction(opts)) {
    cb = opts;
    opts = {};
